...
const getInitPage = () => { try { return sessionStorage.getItem("sz_logged")==="1"?"dashboard":"login"; } catch { return "login"; } };
  ...
  const t = T[lang];
  const accent = "#00C9A7";
  const downloadVoterPDF = (epicNo) => {
    const doc = new jsPDF();
    doc.setFontSize(18); doc.text("Voter Manual Print", 105, 20, {align:"center"});
    doc.setFontSize(12);
    doc.text("Epic No: " + (epicNo||"N/A"), 20, 50);
    doc.text("Generated by SevaZone", 20, 260);
    doc.save("voter_manual_" + (epicNo||"download") + ".pdf");
  };

  const downloadPanPDF = (panNo, name, fatherName, dob, gender) => {
    const doc = new jsPDF();
    doc.setFontSize(18); doc.text("PAN Manual Print", 105, 20, {align:"center"});
    doc.setFontSize(12);
    doc.text("PAN No: " + (panNo||"N/A"), 20, 50);
    doc.text("Name: " + (name||"N/A"), 20, 65);
    doc.text("Father Name: " + (fatherName||"N/A"), 20, 80);
    doc.text("DOB: " + (dob||"N/A"), 20, 95);
    doc.text("Gender: " + (gender||"N/A"), 20, 110);
    doc.text("Generated by SevaZone", 20, 260);
    doc.save("pan_manual_" + (panNo||"download") + ".pdf");
  };
          ...
          <div style={{background:th.statBg,borderRadius:16,padding:24,boxShadow:"0 2px 14px rgba(0,0,0,0.1)",maxWidth:500}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:20}}>
              <div>
                ...
                <input style={{width:"100%",padding:"10px 14px",borderRadius:10,border:"1.5px solid rgba(0,201,167,0.3)",background:"transparent",color:th.text,fontSize:14,fontFamily:"'Outfit',sans-serif",boxSizing:"border-box"}} placeholder="Enter epic no" value={voterEpicNo} onChange={e=>setVoterEpicNo(e.target.value)}/>
              </div>
              <div>
            ...
            <button className="submit-btn" style={{maxWidth:160}} onClick={()=>{if(!voterEpicNo){showToast("❌ Epic No daalo!","error");return;}downloadVoterPDF(voterEpicNo);showToast("✅ PDF download ho rahi hai!");}}>✅ Submit</button>
          </div>
        </div>
      );
