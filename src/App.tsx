...
import { downloadMarksheet12PDF } from "./utils/helpers/pdfGenerators/marksheet12Pdf";
...
import { downloadVoterPDF } from "./utils/helpers/pdfGenerators/voterPdf";
import { downloadAadharPDF } from "./utils/helpers/pdfGenerators/aadharPdf";
import { Toast } from "./components/common/Toast";
import { StatusBadge } from "./components/common/StatusBadge";
import { PanManualForm } from "./components/forms/PanManualForm";
import { Marksheet10Form } from "./components/forms/Marksheet10Form";
import { Marksheet12Form } from "./components/forms/Marksheet12Form";
import { HaryanaDomicileForm } from "./components/forms/HaryanaDomicileForm";
import { NiosForm } from "./components/forms/NiosForm";
import { VoterManualForm } from "./components/forms/VoterManualForm";
import { AadharManualForm } from "./components/forms/AadharManualForm";
...
export default function SevaZone() {
  ...
  const [nEnrolment, setNEnrolment] = useState("");
...
;
  ...
  const downloadNiosPDF = () => {
    ...
    const det = [["Enrolment No","202462961657"],["Course","Secondary"],["Candidate Name",nName||""],["DOB",nDob||""],["Mother's Name",nMother||""],["Father's Name",nFather||""],["Examination Year",nYear||""]];
  ...
  const renderContent = () => {
    ...
    if (activeSubMenu) {
        ...
        return (
          <div className="fade-up">
            ...
            <div style={{background:th.statBg,borderRadius:16,padding:24,boxShadow:"0 2px 14px rgba(0,0,0,0.1)",maxWidth:700}}>
              ...
              {/* Submit */}
              <button className="submit-btn" onClick={()=>initiatePayment(
                "aadhar_manual", "Aadhar Manual Print - ₹10", 1000,
                ()=>{const now=new Date();setAadharPrintList(prev=>[{sn:String(prev.length+1).padStart(3,"0"),aadharNo:"XXXX XXXX XXXX",name:"—",date:now.toLocaleString("en-IN")},...prev]);showToast("✅ Request submit ho gayi!");},
                userName, showToast, setWalletTxns, setRechargeHistory, walletBalance
        ...
        return (
          <div className="fade-up">
            ...
            <div style={{background:th.statBg,borderRadius:16,padding:24,maxWidth:600}}>
              ...
              <button className="submit-btn" style={{maxWidth:240,background:"linear-gradient(135deg,#f59e0b,#d97706)"}} onClick={()=>initiatePayment("voter_manual","Voter Manual Print - ₹20",2000,()=>downloadVoterPDF({vEpic,vName,vPhoto}),userName,showToast,setWalletTxns,setRechargeHistory,walletBalance)}>💳 Pay ₹20 & Download PDF</button>
            </div>
          </div>
        );
             }
                  ...
                  <tbody>{voterPrintList.map((r,i)=>(
                    <tr key={i} style={{borderTop:`1px solid ${th.border}`}}>
                      ...
                      <td style={{padding:"12px 14px"}}>
                        <button onClick={()=>{downloadVoterPDF({vEpic,vName,vPhoto});showToast("✅ Downloading...");}} style={{background:"linear-gradient(135deg,#845EC2,#6A3FA0)",color:"white",border:"none",borderRadius:8,padding:"8px 16px",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"'Outfit',sans-serif"}}>Download Now</button>
                      </td>
                    </tr>
                  ))}</tbody>
                </table>
      ...
      if (activeSubMenu.id === "pan_manual") {
        return (
          <PanManualForm
            pNo={pNo} setPNo={setPNo}
            pName={pName} setPName={setPName}
            pFather={pFather} setPFather={setPFather}
            pDob={pDob} setPDob={setPDob}
            pGender={pGender} setPGender={setPGender}
            pPhoto={pPhoto} setPPhoto={setPPhoto}
            pSign={pSign} setPSign={setPSign}
            onSubmit={() => initiatePayment(
              "pan_manual",
              "PAN Manual Print - ₹30",
              3000,
              async () => await downloadPanTemplatePDF({ pNo, pName, pFather, pDob, pPhoto, pSign }),
              userName,
              showToast,
              setWalletTxns,
              setRechargeHistory,
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                  ...
                  <tbody>{panPrintList.map((r,i)=>(
                    <tr key={i} style={{borderTop:`1px solid ${th.border}`}}>
                      ...
                      <td style={{padding:"12px 14px",color:th.subtext}}>{r.date}</td>
                      <td style={{padding:"12px 14px"}}>
                        <button onClick={()=>{downloadPanTemplatePDF({pNo,pName,pFather,pDob,pPhoto,pSign});showToast("✅ Downloading...");}} style={{background:"linear-gradient(135deg,#2196F3,#1565C0)",color:"white",border:"none",borderRadius:8,padding:"8px 16px",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"'Outfit',sans-serif"}}>Download Now</button>
                      </td>
                    </tr>
                  ))}</tbody>
                </table>
      ...
      if (activeSubMenu.id==="nios_form") return (
        <NiosForm
          nName={nName} setNName={setNName}
          nFather={nFather} setNFather={setNFather}
          nMother={nMother} setNMother={setNMother}
          nDob={nDob} setNDob={setNDob}
          nYear={nYear} setNYear={setNYear}
          nEnrolment={nEnrolment} setNEnrolment={setNEnrolment}
          nPhoto={nPhoto} setNPhoto={setNPhoto}
          onSubmit={() => initiatePayment(
            "nios_form",
            "NIOS Certificate - ₹20",
            2000,
      ...
      if (activeSubMenu.id==="haryana_form") return (
        <HaryanaDomicileForm
          ...
          onSubmit={() => initiatePayment(
            "haryana_form",
            "Haryana Resident Certificate - ₹30",
            3000,
            () => downloadHaryanaDomicilePDF(hy),
            userName,
            showToast,
            setWalletTxns,
            setRechargeHistory,
      ...
      if (activeSubMenu.id==="marksheet_12") return (
        <div className="fade-up">
          ...
          <div style={{background:th.statBg,borderRadius:16,padding:24,maxWidth:700}}>
            ...
            <button className="submit-btn" style={{maxWidth:240,background:"linear-gradient(135deg,#f59e0b,#d97706)"}} onClick={()=>initiatePayment(
  "marksheet_12","12th Marksheet - ₹30",3000,
  ()=>downloadMarksheet12PDF(m12),
  userName,showToast,setWalletTxns,setRechargeHistory,walletBalance
)}>💳 Pay ₹30 & Download PDF</button>
          </div>
        </div>
      );
            // ── OTHER SUB MENUS ──────────────────────
